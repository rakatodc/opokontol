...
function bind(mans) {
    ...
    mans.ev.on('connection.update', update => {
        Object.assign(state, update)
    })

    mans.ev.on('chats.set', ({ chats: newChats, isLatest }) => {
        /**if (isLatest) {
            chats.clear()
        }**/
    ...
    mans.ev.on('contacts.set', ({ contacts: newContacts }) => {
    ...
    mans.ev.on('messages.set', ({ messages: newMessages, isLatest }) => {
        if (isLatest) {
   ...
   mans.ev.on('contacts.update', updates => {
        for (const update of updates) {
            Object.assign(contacts[update.id], update)
        }
    })

    mans.ev.on('chats.upsert', newChats => {
        chats.upsert(...newChats)
    })

    mans.ev.on('messages.delete', item => {
        if ('all' in item) {
    ...
    mans.ev.on('chats.delete', deletions => {
        for (const item of deletions) {
            chats.deleteById(item)
        }
    })

    mans.ev.on('chats.update', updates => {
    ...
    mans.ev.on('presence.update', ({ id, presences: update }) => {
        presences[id] = presences[id] || {}
        Object.assign(presences[id], update)
    })

    mans.ev.on('messages.upsert', ({ messages: newMessages, type }) => {
        switch (type) {
            case 'append':
            case 'notify':
                for (const msg of newMessages) {
    ...
    /**mans.ev.on('messages.update', updates => {
    ...
    mans.ev.on('groups.update', updates => {
        for (const update of updates) {
    ...
    mans.ev.on('group-participants.update', ({ id, participants, action }) => {
    ...
    mans.ev.on('message-receipt.update', updates => {
    ...
    return {
        ...
        loadMessages: async (jid, count, cursor) => {
            ...
            const retrieve = async (count, cursor) => {
                const result = await (mans === null || mans === void 0 ? void 0 : mans.fetchMessagesFromWA(jid, count, cursor))
                return result || []
            }
        ...
        loadMessage: async (jid, id) => {
            ...
            if (!message) {
                message = await (mans === null || mans === void 0 ? void 0 : mans.loadMessageFromWA(jid, id))
            }
            return message
        },
        mostRecentMessage: async (jid) => {
            ...
            if (!message) {
                const [result] = await (mans === null || mans === void 0 ? void 0 : mans.fetchMessagesFromWA(jid, 1, undefined))
                message = result
            }
            return message
        },
        fetchImageUrl: async (jid) => {
            ...
            if (!contact) {
                return mans === null || mans === void 0 ? void 0 : mans.profilePictureUrl(jid)
            }
            if (typeof contact.imgUrl === 'undefined') {
                contact.imgUrl = await (mans === null || mans === void 0 ? void 0 : mans.profilePictureUrl(jid))
            }
            return contact.imgUrl
        },
        fetchGroupMetadata: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (mans === null || mans === void 0 ? void 0 : mans.groupMetadata(jid))
            }
            return groupMetadata[jid]
        },
        fetchBroadcastListInfo: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (mans === null || mans === void 0 ? void 0 : mans.getBroadcastListInfo(jid))
            }
            return groupMetadata[jid]
        },
        fetchMessageReceipts: async ({ remoteJid, id }) => {
            ...
            if (!receipts) {
                receipts = await (mans === null || mans === void 0 ? void 0 : mans.messageInfo(remoteJid, id))
                if (msg) {
